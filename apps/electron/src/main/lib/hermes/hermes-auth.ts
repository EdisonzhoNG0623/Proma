...
import { HermesError, hermesErrorFromHttpStatus, redactSecrets } from './hermes-errors'
...
/** 构建 WS 连接 URL（带 ticket 或 token 查询参数） */
...
export class HermesAuthService {
  ...
  async mintWsTicket(targetId: string): Promise<string> {
    ...
    const cookieHeader = this.serializeCookies(jar)
    const response = await this.transport.requestJson('/api/auth/ws-ticket', {
      method: 'POST',
      headers: cookieHeader ? { Cookie: cookieHeader } : {},
    })
    this.captureCookies(jar, response.headers?.['set-cookie'])

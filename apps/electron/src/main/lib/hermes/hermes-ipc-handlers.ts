  ...
  ipcMain.handle(
    ...
    async (_, input: { targetId: string; protocol?: 'dashboard' | 'api-server'; profile?: string; remoteSessionId?: string; remoteCwd?: string; title?: string; workspaceId?: string }) => {
      ...
      const existing = input.remoteSessionId
        ...
        : undefined
      if (existing) {
        // Reopening an already-bound local session must still refresh from the
        // remote snapshot. Returning early here previously made restart recovery
        // depend on stale local JSONL, so media-only turns disappeared.
        void hermesIpcService
          .hydrateRemoteSessionHistory(existing.id, input.targetId, input.remoteSessionId!, input.profile)
          .then((count) => {
            console.log(`[Hermes] 远端历史已刷新: ${count} 条 -> ${existing.id}`)
          })
          .catch((error) => {
            console.error('[Hermes] 刷新远端历史失败:', error instanceof Error ? error.message : String(error))
          })
        return existing
      }
      ...
      // 已有远端会话：异步拉取历史并分批推送（不阻塞打开；renderer 渐进追加显示）
      if (input.remoteSessionId) {
        void hermesIpcService
          .hydrateRemoteSessionHistory(session.id, input.targetId, input.remoteSessionId, input.profile)
          .then((count) => {
            console.log(`[Hermes] 远端历史已加载: ${count} 条 -> ${session.id}`)
          })
          .catch((error) => {
